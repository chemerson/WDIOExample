const util = require('util'),
    ...
    utils = require('../example/utils.js');
        ...
        if(stats.failures.length) {
            stats.failures.forEach((failure) => {
                ...
                if (config.host.indexOf('saucelabs.com') > -1) {
                    sauceJobId = failure.runningBrowser.split('Check out job at https://saucelabs.com/tests/').pop();
                    sauceJobAuthToken = utils.getSauceJobAuthToken(sauceJobId);
                    browerDetails += `\nSauce Job: ${failure.runningBrowser.split('Check out job at ').pop()}?auth=${sauceJobAuthToken}`;
                }
                attach = {
                    color: '#CD0000',
